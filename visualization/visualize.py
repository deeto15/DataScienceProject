...
def plot_fraud_counts(df, title):
    fraud_counts = df['Class'].value_counts().rename({0: 'Not Fraud', 1: 'Fraud'}).reset_index()
    fraud_counts.columns = ['Class', 'Count']

    plt.figure(figsize=(8, 5))
    ax = sns.barplot(data=fraud_counts, x='Class', y='Count', hue='Class', palette='viridis', legend=False)
...
if __name__ == "__main__":
    ...
    print(f"Fraud counts (raw) plot saved to {os.path.join(OUTPUT_DIR, 'fraud_counts_raw.png')}")
    ...
    print(f"Fraud counts (downsampled) plot saved to {os.path.join(OUTPUT_DIR, 'fraud_counts_downsampled.png')}")
    ...
    print(f"Feature importance (Random Forest) plot saved to {os.path.join(OUTPUT_DIR, 'feature_importance_rf.png')}")
